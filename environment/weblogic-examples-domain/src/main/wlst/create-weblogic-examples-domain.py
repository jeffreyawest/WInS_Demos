...
def createWLDFJMSResources(moduleName, clusterTarget, jmsServerTargets):
    ...
    #### Queue
    cd('/JMSSystemResources/' + moduleName + '/JmsResource/NO_NAME_0')
    topic_name = 'com.oracle.example.jms.wldf.jdbchogger'
    topic = create(topic_name, 'UniformDistributedTopic')
    topic.setJNDIName(topic_name)
    topic.setForwardingPolicy('Replicated')
    topic.setDefaultTargetingEnabled(false)
    ...
    #### Queue
    cd('/JMSSystemResources/' + moduleName + '/JmsResource/NO_NAME_0')
    topic_name = 'com.oracle.example.jms.wldf.stuck_thread_generator'
    topic = create(topic_name, 'UniformDistributedTopic')
    topic.setJNDIName(topic_name)
    topic.setForwardingPolicy('Replicated')
    topic.setDefaultTargetingEnabled(false)
...
try:
    createDomain(WL_HOME + '/common/templates/domains/wls.jar', var_domain_dir, adminServer_Username, adminServer_Password)
...
clusterMBean.setMigrationBasis('consensus')
...
progress = deploy(appName='coherence', path=COHERENCE_HOME + '/lib/coherence.jar', targets=cluster_name, libraryModule='true',
    libImplVersion='3.7.1.1', libSpecVersion='3.7.1.1')
progress.printStatus()

progress = deploy(appName='coherence-web-spi', path=COHERENCE_HOME + '/lib/coherence-web-spi.war', targets=cluster_name,
    libraryModule='true', libImplVersion='1.0.0.0', libSpecVersion='1.0.0.0')
progress.printStatus()

progress = deploy(appName='active-cache', path=WL_HOME + '/common/deployable-libraries/active-cache-1.0.jar', targets=cluster_name,
    libraryModule='true', libImplVersion='1.0', libSpecVersion='1.0')
progress.printStatus()

progress = deploy(appName='weblogic-spring', path=WL_HOME + '/server/lib/weblogic-spring.jar', targets=cluster_name,
