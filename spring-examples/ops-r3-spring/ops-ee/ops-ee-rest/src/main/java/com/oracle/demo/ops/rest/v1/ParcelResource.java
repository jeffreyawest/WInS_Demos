...
import com.oracle.demo.ops.rest.EJBProxy;
...
public class ParcelResource
{
  ...
  public Response testParcelManagerAvailable()
  {
    ...
    try
    {
      ...
      if (ejbs.parcelManager == null)
      {
        error = true;
        errorMessage.append("Parcel Manager EJB is NULL!  ");
      }

      if (ejbs.eventService == null)
      {
        error = true;
  ...
  {
    return ejbs.parcelManager.findAllParcels();
  }
  ...
  {
    return ejbs.parcelManager.getParcelById(id);
  }
  ...
  {
    return ejbs.parcelEventManager.getParcelLogByParcelId(id);
  }
  ...
  {
    List<ParcelEvent> list = ejbs.parcelEventManager.getAllParcelEvents();
  ...
  public Response postLogEvent2(@PathParam("id") int id, ParcelEvent event)
  {
    if (event.getParcelId() == id)
    {
      ...
      ejbs.eventService.sendEventToQueue(event);
