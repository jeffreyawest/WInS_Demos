        ...
        extends javax.ws.rs.core.Application
{
  ...
  /**
   * Retrieves representation of an instance of com.oracle.demo.ops.rest.ParcelServiceResource
   *
  ...
  public String testShipmentManagerAvailable()
  {
    ...
    ShipmentManager shipmentManager = getShipmentManager();
  ...
  {
    ParcelManager parcelManagerLocal = getParcelManager();

    return String.format("ParcelManagerLocal: null:%s , class:%s",
            parcelManagerLocal == null,
            parcelManagerLocal == null ? "null" : parcelManagerLocal.getClass().getName());
  }
  ...
  {
    ShipmentManager sm = getShipmentManager();
  ...
  {
    ParcelManager pm = getParcelManager();
    return pm.findAllParcels();
  }
  ...
  public Parcel createParcel(Parcel parcel)
  {
    System.out.printf("/parcel/create:#Parcel: %s, %s, %s, %s, %s\n",
            ...
            parcel.getHeight(),
            parcel.getLength());
    return getParcelManager().createParcel(parcel);
  }
  ...
  */
  ...
  public Shipment createShipmentFromWizard(Shipment shipment)
  {
    ...
    System.out.printf("/shipment/create1#Shipment: %s [%s], To: %s, From: %s, Parcel: %s\n",
            shipment,
            shipment.getShippingService().getName(),
            shipment.getToAddress(),
            shipment.getFromAddress(),
            shipment.getParcels());
    return getShipmentManager().createShipment(shipment);
  }
  ...
  */
  ...
  {
    ParcelManager parcelManagerLocal;
    try
    {
      InitialContext ic = new InitialContext();
      parcelManagerLocal = (ParcelManager) ic.lookup("java:comp/env/ejb/ParcelManager");
    }
    catch (Exception e)
    {
      e.printStackTrace();
    ...
    return parcelManagerLocal;
  }

  private ShipmentManager getShipmentManager()
  {
    ShipmentManager shipmentManagerLocal;
    try
    {
      InitialContext ic = new InitialContext();
      shipmentManagerLocal = (ShipmentManager) ic.lookup("java:comp/env/ejb/ShipmentManager");
    }
    catch (Exception e)
    {
      e.printStackTrace();
      shipmentManagerLocal = null;
    }
    return shipmentManagerLocal;
  }
}
